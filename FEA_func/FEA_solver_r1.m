...
%% parallel PCG
%         parpool;
%         KG_Parallel = distributed(KG(freedofs,freedofs));
%         F_Parallel = distributed(F(freedofs,1));
%         L_paral = distributed(L);
%         u_old_paral = distributed(u_old(freedofs,1));
%         spmd
%             [U_sparse,flag,relres,iter_temp,rv]  = pcg(KG_Parallel,F_Parallel,tolit,maxit,L_paral,L_paral',u_old_paral);
%         end
%         U_sparse = gather(U_sparse);
%         worker_num = size(iter_temp,2);
%         iter = 0;
%         for iter_loop = 1:worker_num
%             iter = iter + iter_temp{1,iter_loop};
%         end
%         iter = iter / worker_num;
%         delete(gcp);
%%
