...
%
% Input parameter :
% MTOP_aIVG_R10_4s(XL,YL,ZL,elem_size,FE_order,rmin_m,volfrac,data_N,DL_ratio,filter_id,penal_min,elem_type,ex_type,solver_type,logon_id)
...
function MTOP_aIVG_R10_4s(XL,YL,ZL,elem_size,FE_order,rmin_m,volfrac,data_N,DL_ratio,filter_id,penal_min,elem_type,ex_type,solver_type,logon_id)
close all
tic
...
switch ex_type
    ...
    otherwise
        if id_2d == 1
            ...
            DOF_num = 3; %'19.03.31, 3D static, DOFs per node
        end
...
const_num = 1; % number of constraint functions
...
Mnd2_old=100; % grey level indicator initial 
Mnd2=100; % grey level indicator initial
...
while ((kktnorm > kkt_tol && change_obj > tol_obj) ...
         || penal < penal_max || beta < beta_max) && loop < maxloop 
  ...
  if id_2d==1
      ...
      x_phys_FE=reshape(x_phys_FE_1d,nely_d,nelx_d,nelz_d);
  end
  ...
  elseif FE_order==2
      switch ex_type
          case 'HMBB'
              [U]=FE5_2D_quad_HMBB(nelx,nely,KG,Fnew,theta1,u_ini,solver_type);
...
%% Sensitivity & objective normalization
...
%% New grouped previous design variables 
