...
%
% Input parameter :
% MTOP_aIVG_R11_4s(XL,YL,ZL,elem_size,FE_order,rmin_m,volfrac,data_N,DL_ratio,filter_id,penal_min,elem_type,ex_type,solver_type,logon_id)
...
function MTOP_aIVG_R11_4s(XL,YL,ZL,elem_size,FE_order,rmin_m,volfrac,data_N,DL_ratio,filter_id,penal_min,elem_type,ex_type,solver_type,logon_id)
close all
tic
...
if data_N==0 % data_N==0 -> No isosurface variables grouping
    ...
    penal_inc=1.2; % penalty value increment
else
    penal_th=0.0001; % penalty threshold = default 0.05, update : 0.005 ('19.06.30)
    penal_inc=1.2; % penalty value increment    
end
...
beta = 1.0; % projection filter parameter, initial, '19.02.17, 0.01
...
% Mnd2_th = 5.0; % required gray level for beta increasing
eta=0.5; % threshold projection filter parameter (eta)

%% Variable initialize

addpath([pwd,'\\FEA_func']); % FEA functions directory
addpath([pwd,'\\MTOP_func']); % DLSIMP functions directory
...
if ZL==0
    id_2d=1; % 2D identification, 1 = 2d
else
    id_2d=0; % 2D identification, 0 = 3d
end
...
if FE_order == 1
    u_ini = zeros(DOF_num*(nelx+1)*(nely+1),load_num);
elseif FE_order == 2
    u_ini = zeros(DOF_num*(2*nelx+1)*(2*nely+1),load_num);
end


disp('== Parameters (2D topology DLSIMP) ==================')
...
inp_var = struct('XL',XL,...
    ...
    'volfrac',volfrac,'Fnew',Fnew,'ex_type',ex_type,...
    'E_min',E_min,'E',E,'nu',nu,'filter_id',filter_id,...
    'tolx',tolx,'maxloop',maxloop,'elem_type',elem_type,...
...
[H,Hs]=filter_prep_r2(id_2d,inp_filter_prep,x_d);
...
[m,n,xmin,xmax,xold1,xold2,low,upp,a0,a,c_MMA,d] = MMA_param_initialize(data_N2,new_volfrac_ini,const_num);
...
% while ((kktnorm > kkt_tol) || beta_inc_cnt_old > 0 ||...
%         penal_inc_cnt_old > 0 || penal < penal_max || beta < beta_max) && loop < maxloop % '19.03.10, penal max criteria
  ...
  % gray level value
%   Mnd2_th = 5; % gray level indicator threshold
...
%           if (change_obj <= tol_obj || kktnorm <= kkt_tol) && beta<beta_max
...
%               loop_MMA = 1;
              ...
              s0 = 0.5/(beta+1); % lower, upper asymptotes coefficient
  ...
  if loop > 1 && data_N > 0 
      ...
      if data_N2 > data_N_old || data_N2 < data_N_old
          [m,n,xmin,xmax,xold1,xold2,low,upp,a0,a,c_MMA,d,xval] = MMA_param_update(data_N_old,data_N2,xold1,xold2,low,upp,xval,const_num);
  ...
  [c,dc,c_elem]=cal_compliance_2D_lin_r4(x_phys_FE,nelx,nely,penal,U,E,E_min,DL_ratio,I_mat); % '19.06.23, 2D linear
...
%% volume constraint, const. sensitivity
loop_s_time=toc;

if id_2d==1
    [con,coneq,gcon,gconeq,volfrac_eff]=cal_vol_const7(volfrac,nelx_d,nely_d,x_phys_FE);
else
    [con,coneq,gcon,gconeq,volfrac_eff]=cal_vol_const7_3d(volfrac,nelx_d,nely_d,nelz_d,x_phys_FE);
end
...
if data_N==0
    ...
    if filter_id==1  % sensitivity filter
        ...
        dc_f_norm_temp = dc_1d(x_d).*gcon_1d(x_d);        
    end
 
    x_criteria = x_1d(x_d);           
    mod_dc_f = x_criteria.^2.*dc_f_norm_temp;        
end
...
%     sens_map_mat=sub_sens_map_r4_4(mod_dc_f(:),data_N2);
...
if logon_id==1 && data_N>0
    ...
    x_hist=1:1:data_N2;
    figure(21);hist(sens_map_mat,x_hist);grid on;
    title(['Histogram for the number of elements in the group (',num2str(loop),' iter.)']);
    xlabel('group variable number');
    ylabel('number of design variables');
%     axis([0 35 0 25])
%     axis([0 11 0 65])
    f = getframe(gcf);
...
% group_elem_num = zeros(size(xval(eff_x_d),1),1);
% for elem_num_idx=1:size(xval(eff_x_d),1)
%     group_elem_num(elem_num_idx,1) = size(find(sens_map_mat(:)==elem_num_idx),1);
% end
% 
% [xphys_temp,xmma_temp] = OC_func(xval(eff_x_d),df0dx(eff_x_d)/df0dx_w,(dfdx(:,eff_x_d))'/dfdx_w,nelx_d,nely_d,volfrac,H,Hs,2,group_elem_num);
% 
% kktnorm = 10^9;
% low_temp = low(eff_x_d);
% upp_temp = upp(eff_x_d);
...
if id_2d==1
    ...
    xnew     = reshape(xnew_temp,nely_d,nelx_d,nelz_d);
    x_phys_d = x_phys_temp;
end
...
if id_2d==1
    ...
    x_phys = reshape(x_phys,nely_d,nelx_d,nelz_d);
    debug_data(loop,4)=sum(x_phys(:))/(nelx_d*nely_d*nelz_d);
end
...
% PRINT RESULTS
    ...
    if logon_id==1
        if id_2d==1
            save([save_path,'density_result_2d_',num2str(loop),'.mat'],'x_phys','nelx_d','nely_d','XL','YL','loop','Mnd2','beta','xold1','xold2','xTilde','KG','U','dc','dc_f','x','xval','xmma','c','debug_data'); % save iteration result
        else    
            save([save_path,'density_result_3d_',num2str(loop),'.mat'],'x_phys','nelx_d','nely_d','nelz_d','XL','YL','ZL','loop','density_th','Mnd2','beta','xold1','xold2','xTilde','KG','U','xmma','xval','c','debug_data'); % save iteration result
        end
        
    elseif logon_id==2
        if id_2d==1
            save([save_path,'density_result_2d_',num2str(loop),'.mat'],'x_phys','nelx_d','nely_d','XL','YL','loop','Mnd2','beta','U','xmma','c','debug_data'); % save iteration result
        else
            save([save_path,'density_result_3d_',num2str(loop),'.mat'],'x_phys','nelx_d','nely_d','nelz_d','XL','YL','ZL','loop','density_th','Mnd2','beta','U','xmma','c','debug_data'); % save iteration result
        end
...
if logon_id==1
    ...
    if id_2d==1
        ...
        plot_3d_r2(x_phys, density_th, 1.0, 'patch',loop,Mnd2);
     
        f = getframe(gcf);
        imwrite(f.cdata,[save_path,'density_dist_result_3d_',num2str(loop),'.jpg']);
    end
...
if logon_id==0 || logon_id==2
    ...
    if id_2d==1
        ...
        f = getframe(gcf);
        imwrite(f.cdata,[save_path,'density_dist_result_3d_',num2str(loop),'.jpg']);
        save([save_path,'density_result_3d_',num2str(loop),'.mat'],'x_phys','nelx_d','nely_d','nelz_d','XL','YL','ZL','loop','density_th','Mnd2','beta','xold1','xold2','xTilde','KG','U','xmma','xval'); % save iteration result
